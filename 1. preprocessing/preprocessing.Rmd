...
pgs <- read.csv("data/EA_2022_Okbay_curated.PRS.auto.csv") %>% 
  rename("EA22" = "score")

# Merge the EA data with the corresponding participant in each data frame
long_dat <- merge(long_dat, pgs,by.x="PTID",by.y="sample.ID") 
wide_dat <- merge(wide_dat, pgs,by.x="PTID",by.y="sample.ID")
```

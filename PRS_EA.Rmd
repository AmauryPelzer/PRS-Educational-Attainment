...
From this, a person with a negative residual score (below the regression) outperformed his genetic capability for educational attainment (aka overachiever).
...
plot(surv_fit, main = "Kaplan-Meier Survival Curve", xlab = "Time", ylab = "Survival Probability", col = c("blue", "green", "red"), lwd = 2) + legend("topright", legend = c("over-performed", "middle", "under-performed"), col = c("blue", "green", "red"), lwd = 2)
```

```{r}
logrank_test <- survdiff(Surv(as.integer(VISCODE), MMSE_cut) ~ thirtile, data = long_dat)
head(logrank_test)
```
...
```{r}
ggplot(long_dat, aes(x = as.integer(VISCODE), fill = factor(MMSE_cut))) +
  geom_histogram(position = "identity", alpha = 0.6, binwidth = 2) +
  scale_fill_manual(values = c("blue", "red"), labels = c("0", "1")) +
  labs(title = "Overlapped Histograms of Occurrence",
       x = "Time", y = "Frequency") +
  theme_minimal()
...
ggplot(long_dat, aes(x = as.integer(VISCODE), fill = factor(MMSE_cut))) +
  geom_histogram(position = "identity", alpha = 0.6, binwidth = 2) +
  scale_fill_manual(values = c("blue", "red"), labels = c("0", "1")) +
  facet_wrap(~ thirtile, ncol = 1) +
  labs(title = "Overlapped Histograms of Occurrence by Group",
       x = "Time", y = "Frequency") +
  theme_minimal()
```

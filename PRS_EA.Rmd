...
```{r}
x <- lm(PTEDUCAT~PTGENDER, data=wide_dat)
boxplot(PTEDUCAT~PTGENDER, data=wide_dat)

y <- lm(EA22~PTGENDER, data=wide_dat)
...
wide_dat$MMSE_norm <- normMMSE(wide_dat$MMSE) # normalize MMSE
...
wide_dat$MMSE_cut <- ifelse(wide_dat$MMSE_norm < cvalue$MMSE_norm, 1, 0) # Group individuals by cut-off value
...
box1 <- ggplot(filtered_wide_dat, aes(x = factor(Age_Group), y = MMSE)) +
...
box2 <- ggplot(filtered_wide_dat, aes(x = factor(Age_Group), y = MMSE_norm)) +
  geom_boxplot(fill = c("#6d8fa6", "#8596a1","#bbceed"), color = "black") + # Set fill and border colours
  scale_x_discrete(labels = c("60-65", "66-70", "71-75")) + # Set x axis scale labels
  labs(x = "Age Group", y = "MMSE Score") +
  theme_minimal() +
...
res <- lm(MMSE ~ EA22 + Age_Group, data = wide_dat)
res2 <- lm(MMSE_norm ~ EA22 + Age_Group, data = wide_dat)

grid.arrange(box1, box2, ncol = 2)
summary(res)
summary(res2)

rm(filtered_wide_dat, box1, box2, res, res2)
```
...
logrank_test$MMSE_PGS <- wide_dat %>%
...
logrank_test$MMSE_res <- wide_dat %>%
...
logrank_test$MMSE_res_low <- wide_dat %>%
...
logrank_test$MMSE_res_mid <- wide_dat %>%
  filter(as.integer(VISCODE) <= 9, 
         thirtile_res != 2, 
...
logrank_test$MMSE_res_high <- wide_dat %>%
  filter(as.integer(VISCODE) <= 9, 
         thirtile_res != 2, 
...
ggplot(wide_dat %>% filter(!is.na(MMSE_cut)), 
...
logrank_test$MMSE_PGS_low <- wide_dat %>%
  filter(as.integer(VISCODE) <= 9, 
         thirtile_PGS != 2, 
...
logrank_test$MMSE_PGS_mid <- wide_dat %>%
  filter(as.integer(VISCODE) <= 9, 
         thirtile_PGS != 2, 
...
logrank_test$MMSE_PGS_high <- wide_dat %>%
  filter(as.integer(VISCODE) <= 9, 
         thirtile_PGS != 2, 

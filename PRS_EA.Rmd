...
To see if it is necessary to stratify for age groups the 
```{r, echo=FALSE}
filtered_wide_dat <- wide_dat %>% #filter out NA cases (== AGE is not between 60-75)
  filter(!is.na(Age_Group))

# Boxplot for MMSE by Age Group
ggplot(filtered_wide_dat, aes(x = factor(Age_Group), y = MMSE_0)) +
  ...
  ggtitle("Boxplot of MMSE at baseline by Age Group")     # Custom title

res <- lm(MMSE ~ EA22 + Age_Group, data = subset(long_dat, !is.na(Age_Group)))
summary(res)

rm(filtered_wide_dat,res)
```

### MMSE Survival Analysis
The MMSE scores were normalized using the NormPsy package and then the cut-off was calculated.
...
cvalue$MMSE <- wide_dat %>% # Calculate cut-off value for normalized MMSE
    filter(DX_0 == "CN") %>%
    summarize(
      value := mean(MMSE_0, na.rm = TRUE) - 2 * sd(MMSE_0, na.rm = TRUE)
    ) %>%
    pull(value)

long_dat$MMSE_cut <- ifelse(long_dat$MMSE < cvalue$MMSE, 1, 0) # Group individuals by cut-off value
...
logrank_test$MMSE_PGS <- long_dat %>%
  filter(as.integer(VISCODE) <= 9, thirtile_PGS != 1, AGE>=60, AGE<=75) %>%
  survdiff(Surv(as.integer(VISCODE), MMSE_cut) ~ thirtile_PGS, data = .)
logrank_test$MMSE_PGS
...
logrank_test$MMSE_res <- long_dat %>%
  filter(as.integer(VISCODE) <= 9, thirtile_res != 1, AGE>=60, AGE<=75) %>%
  survdiff(Surv(as.integer(VISCODE), MMSE_cut) ~ thirtile_res, data = .)

logrank_test$MMSE_res
```

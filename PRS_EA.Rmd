...
ggplot(wide_dat, aes(x = factor(Age_Group), y = MMSE_0)) +
  geom_boxplot() +
  labs(x = "Age Group", y = "MMSE_0", title = "Boxplot of MMSE_0 by Age Group")
```
...
# normalize MMSE
cvalue <- list()
long_dat$MMSE_norm <- normMMSE(long_dat$MMSE)

cvalue$MMSE <- wide_dat %>%
    filter(DX_0 == "CN") %>%
    summarize(
...
long_dat$MMSE_cut <- ifelse(long_dat$MMSE < cvalue$MMSE, 1, 0)

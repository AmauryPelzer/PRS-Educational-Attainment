...
```{r, include=FALSE}
if (suppressWarnings(!require(tidyverse))) {install.packages("tidyverse"); library(dplyr)}
...
remission_dat <- data.frame(row.names = c("MMSE_cut",
                                          "ADAS11_cut", 
                                          "ADAS13_cut", 
                                          "ADASQ4_cut", 
                                          "CDRSB_cut", 
...
-   Calculating cut-off values using either mean + 2SD or mean - 2\*SD
...
filter_data <- function(outcome_var){
  ...
  remission_dat[paste(outcome_var),"Num"] <- rbind(remission_dat,rem_num) 
}
...
The MMSE scores were normalized using the NormPsy package and then the cut-off was calculated to be `r c_cut$MMSE_norm`
...
#filter out NA cases (== AGE is not between 60-75)
filtered_long_dat <- long_dat %>% 
...
```{r}
cox_filtered_dat <- within(cox_filtered_dat,thirtile_PGS <- relevel(thirtile_PGS, ref="Low PGS")) # Change the Reference Level of the PGS factor
cox_filtered_dat <- within(cox_filtered_dat,thirtile_years <- relevel(thirtile_years, ref="Low EA")) # Change the Reference Level of the PGS factor

mlr_pgs_adas11 <- lme4::lmer(ADAS11 ~ thirtile_PGS + age + PTGENDER + age*PTGENDER + (1 | RID), data=cox_filtered_dat) # Multi-Level Model for PGS
mlr_years_adas11 <- lmer(ADAS11 ~ thirtile_years + age + PTGENDER + age*PTGENDER + (1 | RID), data=cox_filtered_dat) # Multi-Level Model for Years

mlr_models <- list(mlr_pgs_adas11,mlr_years_adas11)
...
# Plot the freq of remission by Cognitive Test

```

# Cross-sectional because... FUCK WHY NOT?!

Does educational attainment or genetic EA affect an individuals risk for MCI at age x?

```{r}
rm(list=setdiff(ls(), "long_dat"))

baseline_df <- long_dat %>% 
  filter(VISCODE == 0 & (DX == "CN"| DX == "MCI"))

summary(lm(MMSE_norm~DX+PTEDUCAT*EA22 + AGE*PTGENDER, data = baseline_df))

baseline_df %>% 
ggplot() +
  geom_violin(aes(x = DX,y =MMSE_norm)) +
  facet_wrap(~thirtile_PGS) +
  stat_summary(aes(x = DX,y =MMSE_norm),
               geom = "point",
               fun = mean)

ggscatter(baseline_df, x = "AGE", y = "MMSE_norm", conf.int = T, cor.coef = T)
```
